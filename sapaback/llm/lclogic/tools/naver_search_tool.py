...
client_id = settings.naver_client_id  # <- 발급받은 client id
client_secret =settings.naver_client_secret       # <- 발급받은 client secret
...
def naver_cafe_search(json_request: str) -> str:
    ...
    '''
    print("naver_cafe_search 호출:" + json_request)
    arguments = json.loads(json_request)
    query = arguments["query"]
    display = arguments["display"]
    print("naver_cafe_search 호출 query:" + query)
    print("naver_cafe_search 호출 displaycount:" + display)
    #required_data = arguments["required_data"]
    return data_retriever(query=query, display=display)
...
request_format = '{{"query":"<query>","display":"<displaycount>"}}'
description = f'''
Helps to retrieve naver cafe search result.
Input should be JSON in the following format: {request_format}
'''

# create an instance of the custom langchain tool
naver_search_tool = Tool(name=name, func=naver_cafe_search, description=description)


if __name__ == '__main__':
    print(naver_cafe_search(where='Genova, Italy', when='today'))
