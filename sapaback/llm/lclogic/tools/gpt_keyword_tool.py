...
#
def keyword_data_retriever(where: str = None, when: str = None, required_data: List[str] = []) -> str:
...
def keyword_tool(json_request: str) -> str:

    print("여기 호출" + "Keyword",json_request)
    '''
    ...
    return keyword_data_retriever(where=where, when=when, required_data=required_data)
...
Weather = Tool(name=name, func=keyword_tool, description=description)
